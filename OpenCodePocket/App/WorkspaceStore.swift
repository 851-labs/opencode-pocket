...
final class WorkspaceStore {
  ...
  private func applyMessagePartDelta(properties: [String: JSONValue]) -> Bool {
    guard
      let mutation = MessageEnvelope.partDeltaMutation(
        from: properties,
    ...
    messagesBySession[mutation.sessionID] = mutation.messages
    return true
  }

  private func applyMessagePartUpdated(properties: [String: JSONValue]) -> Bool {
    guard
      let mutation = MessageEnvelope.partUpdatedMutation(
        from: properties,
        messagesBySession: messagesBySession
      )
    else {
  ...
  private func applyMessageUpdated(properties: [String: JSONValue]) -> Bool {
    guard
      let mutation = MessageEnvelope.messageUpdatedMutation(
        from: properties,
        messagesBySession: messagesBySession
      )
    else {
  ...
  private func applyMessageRemoval(properties: [String: JSONValue]) -> Bool {
    guard
      let mutation = MessageEnvelope.messageRemovalMutation(
        from: properties,
        messagesBySession: messagesBySession
      )
    else {
  ...
  private func applyMessagePartRemoval(properties: [String: JSONValue]) -> Bool {
    guard
      let mutation = MessageEnvelope.partRemovalMutation(
        from: properties,
        messagesBySession: messagesBySession
      )
    else {
