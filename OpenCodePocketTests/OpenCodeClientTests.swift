...
final class OpenCodeClientTests: XCTestCase {
  ...
  func testEndpointsAndSuccessPaths() async throws {
    URLProtocolStub.handler = { request in
      ...
      case let ("POST", path) where path?.hasPrefix("/permission/") == true && path?.hasSuffix("/reply") == true:
        return try makeJSONResponse(request: request, json: "true")
      case ("GET", "/question"):
      ...
      case let ("POST", path) where path?.hasPrefix("/question/") == true && path?.hasSuffix("/reply") == true:
        return try makeJSONResponse(request: request, json: "true")
      case let ("POST", path) where path?.hasPrefix("/question/") == true && path?.hasSuffix("/reject") == true:
        return try makeJSONResponse(request: request, json: "true")
      default:
